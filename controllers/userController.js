...
export const registerUser = async (req, res) => {
  ...
  try {
    const { name, accountname, email, password, WorkspaceName, businessIndustry, businessSize } = req.body;
    ...
    const newWorkspace = await Workspace.create([{
      ...
      owner: user[0]._id,
      admins: [user[0]._id],
      users: [{ user: user[0]._id, role: 'owner' }],
    }], { session });

    // Create JWT and set it in the response
    ...
    res.status(201).json({ user: user[0], workspace: newWorkspace[0] });
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
    console.log(error);
    ...
    try {
        ...
        const token = await new Promise((resolve, reject) => {
            jwt.sign(
                payload,
                'your_jwt_secret',
                { expiresIn: 360000 }, // Token expiry in seconds
                (err, token) => {
                    if (err) {
